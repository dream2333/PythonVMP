...
字节码无法由cpython解释器执行，而仅能由我们自己实现的解释器运行，实现了对python源码的保护

对于外部安全人员来说，未掌握可执行文件格式和指令集的情况下，对该段二进制代码十分难以分析，参考elf的文件格式，我们实现了一个自定义的虚拟机可执行文件格式，包含文件头、常量池、符号表和指令序列。
...
仅仅按照这个可执行文件格式进行解析是不够的，我们还需要按照[指令集定义](./docs/instruction_set.md)来解析指令序列。
...
我们的PyVM实际实现了一个在Python上运行的栈式虚拟机，根据指令集定义，逐条解析指令并执行。
...
这是一个轻量级虚拟机项目，所有功能都使用Python标准库实现，如有额外需求可以查看
