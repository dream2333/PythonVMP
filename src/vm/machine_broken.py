...
class PyVirtualMachine:
    ...
    def run(self) -> None:
        ...
        if self.debug:
            print(f"程序执行完成，共执行 {self.instruction_count} 条指令")
    ...
    def _print_debug_info(self, instruction: Instruction) -> None:
        """打印调试信息"""
        print(f"PC={self.pc:04d}: {instruction}")
    ...
    def _print_program_info(self) -> None:
        """打印程序信息"""
        print(f"常量池 ({len(self.constants)} 项):")
        for i, const in enumerate(self.constants):
            print(f"  [{i}] {const}")
        
        print(f"\n符号表 ({len(self.symbols)} 项):")
        for name, symbol in self.symbols.items():
            print(f"  {symbol}")
        
        print(f"\n指令序列 ({len(self.instructions)} 条):")
    ...
    def _handle_error(self, error: Exception) -> None:
        ...
        print(f"程序计数器: {self.pc}", file=sys.stderr)
        print(f"当前栈状态: {self.stack}", file=sys.stderr)
